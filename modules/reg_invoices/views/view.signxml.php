...
class reg_invoicesViewSignXml extends SugarView{
  ...
  function display(){
    ...
    echo "<div id=\"signpanel\"><p>".translate("LBL_FIRMANDO_$doc", 'reg_invoices')."</p>";
    echo "<p>".translate('LBL_AVISO_JAVA', 'reg_invoices')."</p>";
    
    $version="2.1.0";  // Si se cambia esto, revisar la sección de administración en la que se testea el sistema.
    $dirDoc=trim($sugar_config['site_url'],' /')."/index.php?module=reg_invoices&action={$doc}view&record={$this->bean->id}";
...
<form id="send_signed_doc" action="index.php">
  ...
  <input type="hidden" name="module" value="reg_invoices" />
